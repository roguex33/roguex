...
contract Hypervisor is IMintCallback, ERC20Permit, ReentrancyGuard {
    ...
    {
        sqrtPrice = uint160(FullMath.mulDiv(Math.sqrt(price), 2 ** 96, 1e18));
        sqrtPriceUpper = uint160(
            FullMath.mulDiv(
                Math.sqrt(
        ...
        sqrtPriceUpper = TickMath.getSqrtRatioAtTick(adjustTickUpper);
        sqrtPriceLower = TickMath.getSqrtRatioAtTick(adjustTickLower);
    }
